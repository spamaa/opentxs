...
OTSmartContract::OTSmartContract(const api::Session& api)
    : ot_super(api)
    , m_mapStashes()
    , m_StashAccts(
          std::make_unique<otx::internal::AccountList>(api, Account::stash))
...
//
void OTSmartContract::onActivate(const PasswordPrompt& reason)
{
    ...
    -> UnallocatedCString
{
    ...
    // strNotaryID, &map_Nyms_Already_Loaded))
    //    {
    //        otErr << "OTSmartContract::GetUnitTypeIDofAcct: error:
    //        'From'
        ...
        if (!bSuccessLoadingPartyInbox) {
            ...
            if (!bMoveParty || !bMoveStash) {
                ...
                LogError()(OT_PRETTY_CLASS())(
                    ...
                    (bUnstashing) ? "party"
                                  : "stash")(" failed while performing move.")
                    .Flush();
...
void OTSmartContract::onFinalReceipt(
    ...
    Nym_p theOriginator,
    Nym_p pActingNym,
    const PasswordPrompt& reason)  // AKA "pRemover" in any other
                                   // onFinalReceipt. Could be nullptr.
{
    OTCron* pCron = GetCron();
    ...
    for (auto& it : m_mapParties) {
        ...
        const bool issuedOpening = context.get().VerifyIssuedNumber(opening);
        ...
        if (validOpening) {
            ...
            context.get().CloseCronItem(opening);
...
auto OTSmartContract::ProcessCron(const PasswordPrompt& reason) -> bool
{
    ...
    if (GetHooks(str_HookName, theMatchingClauses)) {
        ...
        ExecuteClauses(theMatchingClauses, reason);
    }
    ...
    UnallocatedSet<OTParty*>* pFailedParties)
{
    ...
    for (auto& it : m_mapParties) {
            ...
            // (So we can skip it here.)
            {
                LogConsole()(OT_PRETTY_CLASS())("FYI, at least one party (")(
                    str_party_name)(") has failed verification, so right now "
                                    "I'm skipping "
                                    "harvesting of his "
                                    "closing transaction numbers. (Since he "
                                    "failed, we "
                                    "never verified his accounts, so we never "
                                    "grabbed "
                                    "those closing "
                                    "numbers in the first place, so there's no "
                                    "need to "
                                    "grab them back now).")
                    .Flush();
                continue;
        ...
        // numbers:
        pParty->HarvestClosingNumbers(strNotaryID, reason);
    }
}
...
void OTSmartContract::UpdateContents(const PasswordPrompt& reason)
{
...
void OTSmartContract::ReleaseLastSenderRecipientIDs()
{
    ...
    m_strLastSenderAcct->Release();  // This is the last Acct ID of a party who
                                     // SENT money.
    m_strLastRecipientUser->Release();  // This is the last Nym ID of a party
                                        // who RECEIVED money.
    ...
    -> std::int32_t
{
    ...
    if (strNodeName->Compare("smartContract")) {
        ...
        const auto strActivatorNymID =
            String::Factory(xml->getAttributeValue("activatorNymID"));
        const auto strActivatorAcctID =
            String::Factory(xml->getAttributeValue("activatorAcctID"));
        const auto strCanceled =
            String::Factory(xml->getAttributeValue("canceled"));
        const auto strCancelerNymID =
    ...
    } else if (strNodeName->Compare("stash"))  // the actual stashes.
    {
        const auto strStashName =
            String::Factory(xml->getAttributeValue("name"));
