...
#include "internal/otx/common/OTTransaction.hpp"  // IWYU pragma: associated
...
//
OTTransaction::OTTransaction(const api::Session& api, const Ledger& theOwner)
    : OTTransactionType(
          api,
          theOwner.GetNymID(),
        ...
        case transactionType::paymentPlan:  // Uses 4 transaction #s: the opener
            ...
            // only marked as "used" upon success.
            {
                ...
                else  // theNym is RECIPIENT / PAYEE
                {
                    ...
                    {
                    public:
                        _getRecipientOpeningNum(const api::Session& api)
                            : api_(api)
                        {
                        }
    ...
    const PasswordPrompt& reason) -> bool
{
    ...
    if (false == bool(pBalanceItem)) {
        ...
        LogConsole()(OT_PRETTY_CLASS())("Wrong type on pBalanceItem (expected "
    ...
    if (!serialized->Exists()) {
        LogConsole()(OT_PRETTY_CLASS())("Unable to load message nym.").Flush();
    ...
    if (nOutboxItemCount != pOutbox->GetTransactionCount()) {
        LogConsole()(OT_PRETTY_CLASS())(
            ...
            " --- nInboxItemCount: ")(
            nInboxItemCount)(" --- nOutboxItemCount: ")(nOutboxItemCount)(".")
            .Flush();
    ...
    for (std::int32_t i = 0; i < pInbox->GetTransactionCount(); i++) {
        ...
        switch (pTransaction->GetType()) {
            ...
            case transactionType::voucherReceipt:   // voucherReceipt has an
                ...
                break;
            default: {
                LogTrace()(OT_PRETTY_CLASS())("Ignoring ")(
...
auto OTTransaction::VerifyBoxReceipt(OTTransaction& theFullVersion) -> bool
{
    ...
    // VERIFY THE HASH
    //
    auto idFullVersion = identifier::Generic{};  // Generate a message digest of
                                                 // that string.
...
auto OTTransaction::VerifyItems(
    ...
    const PasswordPrompt& reason) -> bool
{
    const auto& NYM_ID = theNym.ID();

    if (NYM_ID != GetNymID()) {
        LogError()(OT_PRETTY_CLASS())("Wrong owner passed.").Flush();
        return false;
    }
    ...
    *pbIsSuccess = false;
    ...
    for (auto& it : m_listItems) {
        ...
        switch (pItem->GetType()) {
            ...
            // -----------------------------------------------
            default:
                LogError()(OT_PRETTY_CLASS())("Wrong transaction type passed.")
                    .Flush();
                return false;
        }
    }
...
auto OTTransaction::ProcessXMLNode(irr::io::IrrXMLReader*& xml) -> std::int32_t
{
                                                   ...
                                                   // fixed here.)
    {
        ...
        const auto strNymID = String::Factory(xml->getAttributeValue("nymID"));
...
//
void OTTransaction::UpdateContents(const PasswordPrompt& reason)
{
    ...
    tag.add_attribute("inboxHash", GetInboxHash().asBase58(api_.Crypto()));
...
/*
  ...
  paymentInbox.
 */
void OTTransaction::SaveAbbrevPaymentInboxRecord(
    Tag& parent,
    const PasswordPrompt& reason)
{
    ...
    if (IsAbbreviated()) {
        ...
        auto idReceiptHash =
            identifier::Generic{};           // a hash of the actual transaction
                                             // is stored with its
        CalculateContractID(idReceiptHash);  // abbreviated short-form
                                             // record (in the payment
                                             // inbox, for example.)
        idReceiptHash.GetString(api_.Crypto(), strHash);
    }
...
void OTTransaction::SaveAbbrevExpiredBoxRecord(
    Tag& parent,
    const PasswordPrompt& reason)
{
    Amount lDisplayValue = 0;
...
payments.
 */
void OTTransaction::SaveAbbrevRecordBoxRecord(
    Tag& parent,
    const PasswordPrompt& reason)
{
    ...
    switch (m_Type) {
        ...
        case transactionType::basketReceipt:   // amount is stored on
            ...
            if (IsAbbreviated())               // not the actual value of 0.
            {
                ...
                lAdjustment = GetReceiptAmount(reason);
...
//
void OTTransaction::SaveAbbreviatedNymboxRecord(
    Tag& parent,
    const PasswordPrompt& reason)
{
    Amount lDisplayValue = 0;
...
void OTTransaction::SaveAbbreviatedOutboxRecord(
    Tag& parent,
    const PasswordPrompt& reason)
{
    Amount lAdjustment = 0, lDisplayValue = 0;
        ...
        case transactionType::pending:
            if (IsAbbreviated()) {
                ...
                lDisplayValue =  //    In the outbox, it's already gone.
                    (GetReceiptAmount(reason) * (-1));  // Either way, it will
                                                        // have a 0 adjustment
                                                        // amount, even though
                                                        // perhaps 500 clams
                                                        // display amount.
...
void OTTransaction::SaveAbbreviatedInboxRecord(
    Tag& parent,
    const PasswordPrompt& reason)
{
...
//
void OTTransaction::ProduceInboxReportItem(
    Item& theBalanceItem,
    const PasswordPrompt& reason)
{
    ...
    switch (m_Type) {  // These are the types that have an amount (somehow)
        ...
        default:  // All other types are irrelevant for inbox reports
        {
            LogDebug()(OT_PRETTY_CLASS())("Ignoring ")(GetTypeString())(
...
// added.
//
void OTTransaction::ProduceOutboxReportItem(
    Item& theBalanceItem,
    const PasswordPrompt& reason)
{
    itemType theItemType = itemType::error_state;
    ...
    switch (m_Type) {
        case transactionType::pending:
            ...
            break;
        default:  // All other types are irrelevant for outbox reports.
            LogError()(OT_PRETTY_CLASS())("Error, wrong item type. "
                                          "Returning.")
                .Flush();
            return;
    }
    ...
    switch (GetType()) {  // These are the types that have an amount (somehow)
                                               ...
                                               // item, attached.)
        {
            if (pOriginalItem->GetType() != itemType::depositCheque) {
                LogError()(OT_PRETTY_CLASS())("Wrong item type attached to ")(
...
auto OTTransaction::GetReferenceNumForDisplay() -> std::int64_t
{
    ...
    switch (GetType()) {
        ...
        default:  // All other types have no display reference number -- return
                  // 0.
            return 0;
    }
...
auto OTTransaction::GetSenderNymIDForDisplay(identifier::Nym& theReturnID)
    -> bool
{
        ...
        case transactionType::notice:  // for paymentPlans AND smartcontracts.
        {
            ...
            if (nullptr != pSmart)  // if it's a smart contract...
            {
                ...
                theReturnID = api_.Factory().NymIDFromBase58(
                    pSmart->GetLastSenderNymID().Bytes());
    ...
    if (false == bool(pOriginalItem)) {
        LogError()(OT_PRETTY_CLASS())("Original item not "
...
auto OTTransaction::GetRecipientNymIDForDisplay(identifier::Nym& theReturnID)
    -> bool
{
    if (IsAbbreviated()) { return false; }
    ...
    switch (GetType()) {
                                       ...
                                       // contracts...
        {
            ...
            if (nullptr != pSmart)  // if it's a smart contract...
            {
                ...
                theReturnID = api_.Factory().NymIDFromBase58(
                    pSmart->GetLastRecipientNymID().Bytes());
    ...
    switch (GetType()) {
        case transactionType::transferReceipt: {
            if (pOriginalItem->GetType() != itemType::acceptPending) {
                ...
                // Even though a transfer has no recipient user (just a
                // recipient acct) I still get the Nym ID when he accepts it!
                theReturnID = GetNymID();
                bSuccess = true;
            }
        } break;
                                               ...
                                               // item, attached.)
        {
            ...
            if (!bLoadContractFromString) {
                ...
                // Even though the cheque has no recipient, I still get the Nym
                // ID when he deposits it!
                theReturnID = pOriginalItem->GetNymID();
                bSuccess = true;
            }
        } break;
...
auto OTTransaction::GetSenderAcctIDForDisplay(identifier::Generic& theReturnID)
    -> bool
{
    if (IsAbbreviated()) { return false; }
    ...
    switch (GetType()) {
        case transactionType::paymentReceipt: {
            ...
            if (nullptr != pSmart)  // if it's a smart contract...
            {
                ...
                theReturnID = api_.Factory().IdentifierFromBase58(
                    pSmart->GetLastSenderAcctID().Bytes());
...
auto OTTransaction::GetRecipientAcctIDForDisplay(
    identifier::Generic& theReturnID) -> bool
{
    if (IsAbbreviated()) { return false; }
    ...
    switch (GetType()) {
        case transactionType::paymentReceipt: {
            ...
            if (nullptr != pSmart)  // if it's a smart contract...
            {
                ...
                theReturnID = api_.Factory().IdentifierFromBase58(
                    pSmart->GetLastRecipientAcctID().Bytes());
    ...
    switch (GetType()) {
        ...
        case transactionType::voucherReceipt: {
            if (pOriginalItem->GetType() != itemType::depositCheque) {
                ...
                theReturnID = pOriginalItem->GetPurportedAccountID();  // Here's
                                                                       // the
                // depositor's account
                // ID (even though the
                // cheque was made out
