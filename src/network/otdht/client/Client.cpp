...
#include "network/otdht/client/Client.hpp"  // IWYU pragma: associated
        ...
        switch (type) {
            case Type::sync_ack: {
                ...
                for (const auto& state : ack.State()) {
                    ...
                    namespace bcsync = opentxs::network::otdht;
                    auto data = [&] {
...
auto Client::Imp::process_monitor(Message&& msg) noexcept -> void
{
    ...
    switch (event) {
        case ZMQ_EVENT_HANDSHAKE_SUCCEEDED: {
            ...
            LogConsole()("Connected to otdht server at ")(endpoint).Flush();
            auto& server =
...
auto Client::Imp::process_request(Message&& msg) noexcept -> void
{
    ...
    const auto request = [&] {
        namespace otsync = opentxs::network::otdht;
        const auto proto =
...
auto Client::Imp::process_server(const CString ep) noexcept -> void
{
    if (external_router_.Connect(ep.c_str())) {
        ...
        log_(OT_PRETTY_CLASS())("Connecting to otdht server at ")(ep).Flush();
    } else {
