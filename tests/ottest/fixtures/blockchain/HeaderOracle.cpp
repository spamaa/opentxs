...
auto Test_HeaderOracle_base::verify_post_state(const PostStateVector& vector)
    -> bool
{
    for (const auto& [sHash, spHash, height, status, pStatus] : vector) {
        ...
        if (false == bool(pBlock)) { return false; }
...
auto Test_HeaderOracle_base::verify_siblings(const ExpectedSiblings& vector)
    -> bool
{
...
Test_HeaderOracle_base::~Test_HeaderOracle_base() = default;
