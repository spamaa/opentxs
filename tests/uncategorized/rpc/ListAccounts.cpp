...
TEST_F(RPC_fixture, issuer_accounts)
{
    ...
    const auto expected = [&] {
        ...
        std::transform(
            ...
            [this](const auto& id) { return id.asBase58(ot_.Crypto()); });
...
{
    constexpr auto index{2};
    const auto filterServer = ot_.NotarySession(1).ID().asBase58(ot_.Crypto());
    const auto command =
...
{
    constexpr auto index{2};
    const auto filterServer = ot_.NotarySession(2).ID().asBase58(ot_.Crypto());
    const auto command =
        ot::rpc::request::ListAccounts{index, "", filterServer};
    const auto& list = command.asListAccounts();
    const auto base = ot_.RPC(command);
